...
if run_with_parser:
    ...
    name_config_file = 'df_experiment_regression_2023.csv'
    name_output_var = 'resp_support_days'  # 'resp_support_days'  'oxygen_demand_days'
    name_save_dir = 'results_test'
    part_of_config_df = '6/10'
...
SCALER = main_cross_val.get_scaler(
    'standard_scaler'
)
...
for index, row in df_experiment.iterrows():
    ...
    if str_experiment not in os.listdir(SAVE_DIR):
        ...
        if row['model_type'] == 'poisson_reg':
            MODEL_CONFIG = {
                'model_type': row['model_type'],
                'poisson_max_iter': int(row['poisson_max_iter']),
            }
            # Read configs for parameter distribution
            PARAMETER_DISTRIBUTIONS = {}

            if row['poisson_alpha'] != 'None':
                temp_alpha = row['poisson_alpha'].split('/')
                temp_alpha = [float(x) for x in temp_alpha]
                PARAMETER_DISTRIBUTIONS['alpha'] = uniform(loc=temp_alpha[0], scale=temp_alpha[1])

        elif row['model_type'] == 'random_forest_reg':
            MODEL_CONFIG = {
